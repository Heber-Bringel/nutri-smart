import { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { useForm } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { useAuth } from '../../../viewmodel/auth/AuthViewModel';
import { resetPasswordSchema, ResetPasswordFormData } from '../../../viewmodel/auth/AuthSchema';
import { PageTransition } from '../../components/shared/PageTransition';
import { supabase } from '../../../infra/supabase/client';

export function ResetPasswordPage() {
  const { updatePassword, logout, error, clearError } = useAuth();
  const navigate = useNavigate();
  const [submitting, setSubmitting] = useState(false);
  // Controla se há uma sessão de recuperação válida (vinda do link do e-mail).
  const [validSession, setValidSession] = useState<boolean | null>(null);

  useEffect(() => {
    // O Supabase detecta o token de recuperação na URL automaticamente
    // (detectSessionInUrl) e cria uma sessão temporária. Verificamos se ela
    // existe para decidir se o formulário pode ser exibido.
    let active = true;

    const checkSession = async () => {
      const { data: { session } } = await supabase.auth.getSession();
      if (active) setValidSession(!!session);
    };

    // Escuta o evento PASSWORD_RECOVERY, que dispara quando o link é aberto.
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      if (event === 'PASSWORD_RECOVERY' || session) {
        if (active) setValidSession(true);
      }
    });

    checkSession();

    return () => {
      active = false;
      subscription.unsubscribe();
    };
  }, []);

  const {
    register,
    handleSubmit,
    formState: { errors },
  } = useForm<ResetPasswordFormData>({
    resolver: zodResolver(resetPasswordSchema),
    defaultValues: { password: '', confirmPassword: '' },
  });

  const onSubmit = async (data: ResetPasswordFormData) => {
    if (submitting) return;
    clearError();
    setSubmitting(true);
    try {
      await updatePassword(data.password);
      // Encerra a sessão de recuperação e redireciona ao login com confirmação.
      sessionStorage.setItem('logout_voluntario', 'true');
      await logout();
      navigate('/login?resetSuccess=true', { replace: true });
    } catch {
      // Erro exibido via `error` do AuthViewModel.
      setSubmitting(false);
    }
  };

  const inputStyle: React.CSSProperties = {
    width: '100%',
    padding: '10px 12px',
    borderRadius: 'var(--radius-md)',
    border: '1px solid var(--color-border)',
    fontSize: 14,
    color: 'var(--color-ink-primary)',
    background: 'var(--color-surface)',
    outline: 'none',
    boxSizing: 'border-box',
    transition: 'border-color 150ms ease-out',
  };

  const labelStyle: React.CSSProperties = {
    display: 'block',
    fontSize: 11,
    fontWeight: 500,
    color: 'var(--color-ink-secondary)',
    marginBottom: 6,
    letterSpacing: '0.05em',
    textTransform: 'uppercase',
  };

  return (
    <PageTransition>
      <div style={{
        display: 'flex',
        justifyContent: 'center',
        alignItems: 'center',
        minHeight: '100vh',
        background: 'var(--color-bg)',
      }}>
        <div style={{
          width: '100%',
          maxWidth: 380,
          padding: '40px 36px',
          borderRadius: 'var(--radius-lg)',
          background: 'var(--color-surface)',
          border: '1px solid var(--color-border)',
        }}>
          <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 4 }}>
            <img
              src="/nutrismart-logo.png"
              alt="NutriSmart"
              style={{ height: 80, width: 'auto', objectFit: 'contain' }}
            />
          </div>
          <h1 style={{
            textAlign: 'center',
            fontSize: 20,
            fontWeight: 700,
            color: 'var(--color-ink-primary)',
            margin: '0 0 4px',
            letterSpacing: '-0.02em',
          }}>
            Redefinir senha
          </h1>
          <p style={{ textAlign: 'center', color: 'var(--color-ink-secondary)', fontSize: 13, marginBottom: 28, marginTop: 0 }}>
            Escolha uma nova senha para sua conta
          </p>

          {validSession === false ? (
            <div style={{
              padding: '12px 14px',
              borderRadius: 'var(--radius-md)',
              background: 'var(--color-danger-subtle)',
              border: '1px solid var(--color-danger-border)',
              color: 'var(--color-danger)',
              fontSize: 12,
              textAlign: 'center',
            }}>
              O link de redefinição é inválido ou expirou. Solicite um novo link na
              tela de recuperação de senha.
            </div>
          ) : (
            <>
              {error && (
                <div style={{
                  padding: '10px 14px',
                  marginBottom: 20,
                  borderRadius: 'var(--radius-md)',
                  background: 'var(--color-danger-subtle)',
                  border: '1px solid var(--color-danger-border)',
                  color: 'var(--color-danger)',
                  fontSize: 12,
                }}>
                  {error}
                </div>
              )}

              <form onSubmit={handleSubmit(onSubmit)}>
                <div style={{ marginBottom: 16 }}>
                  <label style={labelStyle}>Nova senha</label>
                  <input
                    type="password"
                    placeholder="••••••••"
                    disabled={submitting}
                    style={{ ...inputStyle, borderColor: errors.password ? 'var(--color-danger)' : 'var(--color-border)', opacity: submitting ? 0.6 : 1 }}
                    onFocus={(e) => { if (!submitting) e.target.style.borderColor = 'var(--color-primary)'; }}
                    {...register('password')}
                  />
                  {errors.password && (
                    <span style={{ color: 'var(--color-danger)', fontSize: 11, marginTop: 4, display: 'block' }}>
                      {errors.password.message}
                    </span>
                  )}
                </div>

                <div style={{ marginBottom: 24 }}>
                  <label style={labelStyle}>Confirmar nova senha</label>
                  <input
                    type="password"
                    placeholder="••••••••"
                    disabled={submitting}
                    style={{ ...inputStyle, borderColor: errors.confirmPassword ? 'var(--color-danger)' : 'var(--color-border)', opacity: submitting ? 0.6 : 1 }}
                    onFocus={(e) => { if (!submitting) e.target.style.borderColor = 'var(--color-primary)'; }}
                    {...register('confirmPassword')}
                  />
                  {errors.confirmPassword && (
                    <span style={{ color: 'var(--color-danger)', fontSize: 11, marginTop: 4, display: 'block' }}>
                      {errors.confirmPassword.message}
                    </span>
                  )}
                </div>

                <button
                  type="submit"
                  disabled={submitting}
                  style={{
                    width: '100%',
                    padding: '10px 24px',
                    borderRadius: 'var(--radius-md)',
                    border: 'none',
                    background: submitting ? 'var(--color-ink-tertiary)' : 'var(--color-primary)',
                    color: '#fff',
                    fontSize: 12,
                    fontWeight: 600,
                    cursor: submitting ? 'not-allowed' : 'pointer',
                    letterSpacing: '0.05em',
                    textTransform: 'uppercase',
                    transition: 'background 150ms ease-out',
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'center',
                    gap: 8,
                  }}
                  onMouseEnter={(e) => { if (!submitting) e.currentTarget.style.background = 'var(--color-primary-hover)'; }}
                  onMouseLeave={(e) => { if (!submitting) e.currentTarget.style.background = submitting ? 'var(--color-ink-tertiary)' : 'var(--color-primary)'; }}
                >
                  {submitting && (
                    <svg
                      width="14" height="14" viewBox="0 0 24 24" fill="none"
                      stroke="currentColor" strokeWidth="2.5"
                      style={{ animation: 'spin 0.7s linear infinite' }}
                    >
                      <circle cx="12" cy="12" r="10" strokeOpacity="0.3" />
                      <path d="M12 2a10 10 0 0 1 10 10" />
                    </svg>
                  )}
                  {submitting ? 'Salvando...' : 'Redefinir senha'}
                </button>
              </form>
            </>
          )}
        </div>

        <style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style>
      </div>
    </PageTransition>
  );
}
